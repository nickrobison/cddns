...
let _sexp_of_ctx _ = Sexplib0.Sexp.List [] (* TODO? *)

let _default_ctx ?headers ?body ?chunked _ _ = Lwt.fail_with "mock"
[@@ocaml.warning "-27"]
...
let ctx_of_router router ?headers ?body ?chunked meth uri =
  ...
  let%lwt status, body =
    ...
    | NoMatch -> Lwt.return (`Not_found, "Not found")
    | FullMatch r -> r ?headers ?body meth
    | MatchWithTrailingSlash r -> r ?headers ?body meth
  in
  Lwt.return
...
let head ?ctx ?headers uri = Lwt.Infix.(call ?ctx ?headers `HEAD uri >|= fst)
...
let post_form ?ctx ?headers ~params uri = Lwt.fail_with "TODO post_form"
[@@ocaml.warning "-27"]

let callv ?ctx uri req = Lwt.fail_with "TODO callv" [@@ocaml.warning "-27"]
